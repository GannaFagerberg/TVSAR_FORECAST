...
p_max = [p_max_ar, p_max_ma]

# ============================================================
# Presample observations for AR initialization
# ============================================================

if model_type in (:SAR, :SARMA) && !SAR_conditional

    n_init = p_max_ar

    init_start = train_start - Hour(n_init)
    init_end   = train_start - Hour(1)

    df_init = select_period(
        df,
        init_start,
        init_end
    )

    @assert nrow(df_init) == n_init

    y_init = Float64.(df_init.demand_VIC)
    timestamp_init = df_init.datetime

    @show n_init
    @show first(timestamp_init)
    @show last(timestamp_init)
