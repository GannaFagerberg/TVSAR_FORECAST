...
function UpdateErrorVolatility_DSP(
    ...
    σ̄²ₙ = 1, d_order = d_order, offsetSV = eps()
)
...
function UpdateErrorVolatility_DSP_ref(
    errors, h̄, hstar, h̃, ξ̄, ϕ̄, μ̄,
    ϕ̄₀, κ̄₀, m̄₀, σ̄₀, ν̄₀, ψ̄₀,
    mixLogχ²₁, postDist, m, v, Ssv;
    σ̄²ₙ = 1, offsetSV = eps()
)
    ...
    hstar = Update_h_with_h0(
        ...
        1.0 ./ exp.(h̄);        # precision = exp(-h_t)
        σ²ₙ = 1.0,
        m̄₀ = m̄₀_emp,               # prior mean for h*_0
        σ̄₀ = 1.0                # prior sd for h*_0
    )
...
function UpdateErrorVolatility_DSP_ref(
    ...
    # ȳstar = log.(errors.^2 .+ offsetSV) without temporaries
    ȳstar = similar(errors, Float64)
    @inbounds @simd for i in eachindex(errors)
        ȳstar[i] = log(errors[i]^2 + offsetSV)
    end

    # mixture allocation for log χ²₁ (same logic, but with views to avoid copies)
    @views h_prev = hstar[2:end]
    s̄ = UpdateMixAlloc(ȳstar, h_prev, mixLogχ²₁)

    #m̄₀_emp = median(ȳstar[1:10]) 
    m̄₀_emp = ȳstar[1]
    # Update hstar including h0 (same outputs)
    hstar = Update_h_with_h0(
        ȳstar, m[s̄], v[s̄], 1.0 ./ exp.(h̄);
        σ²ₙ = 1.0,
        m̄₀ = m̄₀_emp,
        σ̄₀ = 1.0
    )

    # omega = diff(hstar, dims=1) (kept identical in meaning)
    omega = diff(hstar)

    # Update SV block (same call/returns)
    h̄, h̃, Ssv, ξ̄, ϕ̄, μ̄ = update_dsp_sv_h!(
        omega, Ssv, h̄, h̃, ξ̄, ϕ̄, μ̄,
        ϕ̄₀, κ̄₀, m̄₀, σ̄₀, ν̄₀, ψ̄₀,
        mixLogχ²₁, vec(m), vec(v), postDist;
...
function Update_h_with_h0_ref(
    ...
    # Build precision Q for [h0; h1..hT] directly (tridiagonal + obs + prior)
    # From D' * W * D where W = diag(invx):
    # Q[1,1]       = invx[1]
    # Q[i,i]       = invx[i-1] + invx[i]  for i=2..T
    # Q[T+1,T+1]   = invx[T]
    # Q[i,i+1]     = -invx[i]  for i=1..T
    # Add prior on h0: + 1/σ̄₀^2 at (1,1)
    # Add observation precision invv[t] to h_t (indices 2..T+1)
    fill!(diagQ, 0.0)
    fill!(offQ, 0.0)

    @inbounds begin
        diagQ[1] = invx[1] + 1.0 / (σ̄₀^2)
        for i in 2:T
            diagQ[i] = invx[i - 1] + invx[i]
        end
        diagQ[T̃] = invx[T]
        ...
        # add observation precision to indices 2..T+1 (h1..hT)
        for t in 1:T
            diagQ[t + 1] += invv[t]
        end
    end

    # Sparse symmetric tridiagonal
    Q = spdiagm(-1 => offQ, 0 => diagQ, 1 => offQ)
    Q = PDSparseMat(Q)
    ...
    # return [h0; h] exactly as before (length T+1)
    return h_all
end

function update_dsp_sv_h_ref!(
    ν, S, H, H̃, ξ, ϕ, μ,
    ϕ₀, κ₀, m₀, σ₀, ν₀, ψ₀,
    mixLogχ²₁, m, v, postDist;
    offset = eps(), α = 1/2, β = 1/2, σ²ₙ = 1.0
    ...
    # Ỹ = log.(ν.^2 .+ offset) without extra temporaries
    Ỹ = similar(ν, Float64)
    @inbounds @simd for i in eachindex(ν)
        Ỹ[i] = log(ν[i]^2 + offset)
    end

    # Cache Dᵩ by T (constructed once, subdiag overwritten each call)
    Dᵩ = _get_Dphi(T)

    # Update mixture allocation for log χ²₁ distribution
    # (kept identical to your current call)
    S=UpdateMixAlloc(Ỹ, H, mixLogχ²₁)

    # Update h₁..h_T
    H = UpdateH_sv(Ỹ, m[S], v[S], Dᵩ, ξ, ϕ, μ; σ²ₙ = σ²ₙ)

    # Update Polya-Gamma variables
    #ξ = Updateξ(H, ϕ, σ²ₙ, μ, α, β)
    ξ = Updateξ_sv(H, ϕ, σ²ₙ, μ, α, β; δ = 0.0)
    ...
    # Transform back to non-centered parameterization
    H̃ = H .- μ
...
function UpdateH_sv_ref(ỹ, m, v, Dᵩ, ξ, ϕ, μ;  σ²ₙ=1)
    T = length(ỹ)
    @views Dᵩ[band(-1)] .= -ϕ

    invΣₓ = Diagonal(ξ ./σ²ₙ)
    invΣᵥ = Diagonal(1.0 ./vec(v))
    Qh̃ = PDSparseMat(sparse(invΣᵥ + Dᵩ' * invΣₓ * Dᵩ))
    lh̃ = invΣᵥ*(ỹ .- m .- μ)

    return rand(MvNormalCanon(vec(lh̃), Qh̃)) .+ μ
end



function UpdateH_sv_orig(ỹ, m, v, Dᵩ, ξ, ϕ, μ; σ²ₙ=1.0)
...
function UpdateH_sv_ref(ỹ, m, v, Dᵩ, ξ, ϕ, μ; σ²ₙ=1.0)
